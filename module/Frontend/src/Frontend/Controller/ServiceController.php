...
class ServiceController extends MasterController
{
    public function categoryAction()
    {
        ...
        $result = $postModel->fetchPage([
            ...
            'WHERE' => 'post_type = 3 AND post.category_id =' . $postCategoryId,
            'page' => $page,
            'pageItemPerCount' => 20,
        ]);
    ...
    public function detailAction()
    {
        ...
        $postOther = $postModel->fetchWhere(['WHERE' => 'post_status = 1 AND category_id = ' . $post['category_id'] . ' AND post_type = 3 AND post_id <>' . $post['post_id']]);
        $escaper = new \Zend\Escaper\Escaper('utf-8');

        $crum = '<ul>
                    <li><a href="/">' . $this->translator->translate('home') . '</a></li>
                    <li><a href="' . $url('home-service-category', array('name' => $functions->formatTitle($postCategory['category_name_' . $this->lang]), 'id' => $postCategory['category_id'])) . '">' . $escaper->escapeHtml($postCategory['category_name_' . $this->lang]) . '</a></li>
        ...
        $websiteGeneral['website_general_url'] = $url('home-service-detail', array('name' => $functions->formatTitle($post['post_title_' . $this->lang]), 'id' => $post['post_id']));
